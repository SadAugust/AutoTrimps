...
function drawGrid(maps) {
	...
	const eggCell = document.getElementsByClassName('eggCell');
	if (eggCell && eggCell[0]) eggCell[0].addEventListener('click', easterEggClicked);
}
...
function updateAllBattleNumbers(skipNum) {
	...
	if (!skipNum && game.global.challengeActive == 'Trimp' && game.jobs.Amalgamator.owned > 0) elemText = toZalgo(prettify(game.resources.trimps.getCurrentSend()), game.global.world);
	else if (!skipNum) elemText = prettify(game.resources.trimps.getCurrentSend());

	if (elemText !== prettify(cell.maxHealth) && elem.innerHTML !== elemText.toString()) elem.innerHTML = elemText;
...
function startFight() {
	...
	if (badGuyName.innerHTML !== badName.toString()) badGuyName.innerHTML = badName;
...
function drawAllUpgrades(force) {
	...
	for (const item in upgrades) {
		...
		innerHTML += drawUpgrade(item, elem);
	}

	if (elem.innerHTML !== innerHTML) elem.innerHTML = innerHTML;
	if (elem.innerHTML !== '' && game.options.menu.showAlerts.enabled) {
		const upgradeAlert = document.getElementById('upgradesAlert');
		if (upgradeAlert.innerHTML !== '!') upgradeAlert.innerHTML = '!';
	}
...
function drawUpgrade(what) {
	...
	let html;
	const alertMessage = game.options.menu.showAlerts.enabled ? '!' : '';
...
function checkAndDisplayResources() {
	...
	for (const item in game.resources) {
		...
		elemText = prettify(Math.floor(toUpdate.owned));
		if (elem.innerHTML !== elemText.toString()) elem.innerHTML = elemText;
...
function checkAndDisplayBuildings() {
	...
	for (const item in game.buildings) {
		...
		elemText = game.options.menu.menuFormatting.enabled ? prettify(toUpdate.owned) : toUpdate.owned;
		if (elem.innerHTML !== elemText.toString()) elem.innerHTML = elemText;
		if (item === 'Trap') {
