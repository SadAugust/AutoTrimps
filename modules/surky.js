...
function initialLoad(skipLevels = false) {
    ...
    if (game.global.exterminateDone || props.specialChallenge === 'downsize') {
...
function iterateValueFeedback(valueArray, props, perks) {
    ...
    } else if (props.specialChallenge === 'trappa' || props.specialChallenge === 'combat' && challengeActive('Trappapalooza')) {
    ...
    var pushPower = Va * ((props.specialChallenge === 'combat' && challengeActive('Trappapalooza')) ? 1 : Vh) / Vpushed;
    ...
    valueArray[15] = Va * ((props.specialChallenge === 'combat' && challengeActive('Trappapalooza')) ? 1 : Vh);
    valueArray[16] = Vm;
    valueArray[17] = Vf;
    valueArray[18] = Vres;
    valueArray[19] = Vp;
...
function getLogWeightedValue(props, perks, Va, Vh, Vgear, Vres, Vrad, Ve = 1, Vp = 1, moreTrinkets = 0, extraObs = 0) {
    ...
    var Wh = (props.specialChallenge === 'combat' && challengeActive('Trappapalooza')) ? 0 : props.clearWeight * props.healthDerate + props.survivalWeight; // health weight
    ...
    var valueArray = [Va, Vh, Vgear, 1, Vres, Vrad, Vp, Ve,
        props.tributes, props.housingCount, game.global.exterminateDone, props.meteorologists,
        obsDropRate(obsLevel, props.targetZone, props) / 100, props.baselineTrinketsNext, obsLevel,
        1, 1, 1, 1, 1,
    ];

    for (var i = 0; i < iterateValueLoops; i++)
        iterateValueFeedback(valueArray, props, perks);
...
function getPerkEfficiencies(props, perks) {
    ...
    var tauntCorrectedMaxTrimps = tauntMult * ((game.resources.trimps.max * game.resources.trimps.maxMod) * autoBattle.bonuses.Scaffolding.getMult()) * Math.pow(tauntBase, props.imperzone * (props.targetZone - game.global.world));
    ...
    if (props.specialChallenge === 'trappa' && challengeActive('Trappapalooza')) {
    ...
    if (props.specialChallenge === 'trappa' || props.specialChallenge === 'combat' && challengeActive('Trappapalooza')) {
    ...
    if (props.specialChallenge === 'downsize') {
        ...
        var buildPop = (game.global.exterminateDone ? (13 + !autoBattle.oneTimers.Collectology.owned ? 1 : autoBattle.oneTimers.Collectology.getHubs()) : 7) * props.housingCount;
        // 2 territory bonuses per zone, each bonus is 5 + trumps level
    ...
    if (props.specialChallenge !== 'berserk') {
        var frenzyHits = 300;
        var isDeathless = props.specialChallenge === 'resplus' || props.specialChallenge === 'resminus' || props.specialChallenge === 'trappa' || props.specialChallenge === 'combat' && challengeActive('Trappapalooza');
    ...
    perks.Prismal.efficiency = getLogWeightedValue(props, perks, ((props.specialChallenge === 'combat' && challengeActive('Trappapalooza')) ? prismGain : 1), prismGain, 1, 1, 1, 1) / getPerkCost('Prismal', 1, false, perks);
    ...
    if (props.perksRadon > 0) {
        ...
        if (props.specialChallenge === 'trappacarp') {
            ...
            while (continueBuying)
                [continueBuying, props, perks] = buyPerk('Carpentry', 1, props, perks);
            props.radonSpent = getTotalPerksCost(perks);
...
function autobuyPerks(props, perks) {
    ...
    perks.Bait.optimize = (props.specialChallenge === 'trappa' || props.specialChallenge === 'combat' && challengeActive('Trappapalooza'));
    perks.Pheromones.optimize = (game.stats.highestRadLevel.valueTotal() >= 60) && (props.specialChallenge !== 'trappa') && !(props.specialChallenge === 'combat' && challengeActive('Trappapalooza'));
    if (props.specialChallenge === 'trappa' || props.specialChallenge === 'combat' && challengeActive('Trappapalooza')) {
        ...
        props.trappaStartPop = 10 * Math.pow(1.1, maxCarpLevels) * autoBattle.bonuses.Scaffolding.getMult();
    }
    ...
    if (!(props.specialChallenge === 'combat') && !(props.specialChallenge === 'combatRadon')) {
        while (continueBuying)
            [continueBuying, props, perks] = buyPerk('Trumps', 1, props, perks);
    }
    continueBuying = true;
    // and Pheromones! (but not in Trappa, for minimum confusion, and not before Trappa unlock)
    if (props.specialChallenge !== 'trappa' && !(props.specialChallenge === 'combat' && challengeActive('Trappapalooza'))) {
        while (continueBuying)
            [continueBuying, props, perks] = buyPerk('Pheromones', 1, props, perks);
    }
    continueBuying = true;
    // secret setting to dump remaining Rn into bait for feeeeeee
    while (continueBuying)
        [continueBuying, props, perks] = buyPerk('Bait', 1, props, perks);
