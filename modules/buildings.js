function safeBuyBuilding(building, amt) {
    if (!building || !amt) return;
    if (isBuildingInQueue(building)) return;
    if (game.buildings[building].locked) return;
    if (!canAffordBuilding(building, false, false, false, false, amt)) return;
...
function buyStorage(hypoZone = 0) {
    ...
    for (var resource in buildings) {
        ...
        if (game.global.mapsActive) {
            ...
            exoticValue = map.name === 'Atlantrimp' || map.name === 'Trimple Of Doom' ? curRes : game.unlocks.imps.Jestimp ? scaleToCurrentMap(simpleSeconds(buildings[resource], 45)) : game.unlocks.imps.Chronoimp ? scaleToCurrentMap(simpleSeconds(buildings[resource], 5)) : exoticValue;
        }
        ...
        if ((game.global.world === 1 && curRes > maxRes * 0.7) || (game.global.world >= 2 && game.global.world < 10 && curRes > maxRes * 0.5) || curRes + exoticValue > maxRes * 0.85) {
...
function mostEfficientHousing() {
    ...
    for (var resource in resourceTypes) resourcePerSecond[resourceTypes[resource]] = getPsString_AT(resourceTypes[resource]);
...
function buyBuildings() {
    ...
    if (challengeActive('Hypothermia')) {
        ...
        if (hypoSettings[0].active && hypoSettings[0].autostorage && hypoSettings.length > 0) {
            for (var y = 1; y < hypoSettings.length; y++) {
                if (!hypoSettings[y].active) continue;
                if (hypoZone === 0 || hypoZone > hypoSettings[y].world) hypoZone = hypoSettings[y].world;
            }
        }
    }
    ...
    if (game.global.world < hypoZone && game.global.autoStorage) toggleAutoStorage(false);

    //Buys storage buildings when about to cap resources
    if (!game.global.improvedAutoStorage) buyStorage(hypoZone);
    ...
    if (game.global.universe === 1) {
        ...
        if (!game.buildings.Gym.locked && buildingSettings.Gym && buildingSettings.Gym.enabled) {
            ...
            if (gymAmt > game.buildings.Gym.purchased && gymCanAfford > 0) {
                if (!needGymystic()) safeBuyBuilding('Gym', gymCanAfford);
            }
        }
...
function buyTributes() {
    ...
    if (game.global.universe === 2) {
        ...
        if (jobSettings.Meteorologist.enabled || mapSettings.shouldTribute) {
            affordableMets = getMaxAffordable(game.jobs.Meteorologist.cost.food[0] * Math.pow(game.jobs.Meteorologist.cost.food[1], game.jobs.Meteorologist.owned), game.resources.food.owned * (jobSettings.Meteorologist.percent / 100), game.jobs.Meteorologist.cost.food[1], true);
        }
    }
...
function buyHousing(buildingSettings) {
    ...
    if (mapSettings.mapName === 'Smithy Farm' && housing !== 'Gateway') return false;
    ...
    if (mapSettings.mapName === 'Tribute Farm' && !mapSettings.buyBuildings && (housing !== 'Gateway' || housing !== 'Collector')) return false;
