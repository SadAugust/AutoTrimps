...
function initializeAllSettings() {
	...
	if (displayCore) {
		...
		createSetting('radonsettings', ['Helium', 'Radon'], 'Switch between Helium (U1) and Radon (U2) settings. ', 'multitoggle', 0, null, 'Core', [1, 2]);
		...
		//Pause + Switch
		createSetting('PauseScript', 'Pause AutoTrimps', 'Pause AutoTrimps Script (not including the graphs module)', 'boolean', null, null, 'Core', [1, 2]);
		...
		createSetting('AutoEggs', 'AutoEggs', 'Click easter egg if it exists, upon entering a new zone. Warning: Quite overpowered. Please solemnly swear that you are up to no good.', 'boolean', false, null, 'Core', [1, 2],
	...
	if (displayDaily) {
		...
		createSetting('bloodthirstDestack', 'Bloodthirst Destack', 'Will automatically run a level 6 map when you are one stack (death) away from the enemy healing and gaining additional attack. <b>Won\'t function properly without Auto Maps enabled.</b>', 'boolean', false, null, 'Daily', [2]);
		...
		createSetting('dailyPortal', ['Daily Portal Off', 'DP: He/Hr', 'DP: Custom'], '\
	...
	<br><b>DP: Custom:</b> Portals into this challenge at the zone you have defined in Daily Custom Portal.', 'multitoggle', '0', null, 'Daily', [1, 2]);

		createSetting('dailyHeliumHourChallenge', 'DP: Challenge', 'Automatically portal into this challenge when using helium per hour or custom autoportal in dailies when there are none left. Custom portals on the zone specified in \'Daily Custom Portal\'. Do not choose a challenge if you havent unlocked it. ', 'dropdown', 'None', ['None', 'Balance', 'Decay', 'Electricity', 'Life', 'Crushed', 'Nom', 'Toxicity', 'Watch', 'Lead', 'Corrupted', 'Domination', 'Experience'], 'Daily', [1, 2],
			function () { return (getPageSetting('dailyPortal', currSettingUniverse) > 0) });

		createSetting('dailyC2Challenge', 'DP: C2', 'Automatically portal into this challenge when using helium per hour or custom autoportal in dailies when there are none left. Custom portals at the zone specified.', 'dropdown', 'None', challenge2, "Daily", [1, 2],
		...
		createSetting('dailyPortalZone', 'Daily Custom Portal', 'Automatically portal at this zone during dailies. (ie: setting to 200 would portal when you reach zone 200)', 'value', '999', null, 'Daily', [1, 2],
			function () { return (getPageSetting('dailyPortal', currSettingUniverse) >= 2) });

		createSetting('dailyDontPortalBefore', 'D: Don\'t Portal Before', 'Do NOT allow Helium per Hour Daily AutoPortal setting to portal BEFORE this level is reached in dailies. It is an additional check that prevents drops in helium/hr from triggering autoportal in dailies. Set to 0 or -1 to completely disable this check. (only shows up with Helium per Hour set in dailies)', 'value', '999', null, 'Daily', [1, 2],
			function () { return (getPageSetting('dailyPortal', currSettingUniverse) === 1) });

		createSetting('dailyHeliumHrBuffer', 'D: He/Hr Portal Buffer %', 'IMPORTANT SETTING. When using the Daily He/Hr Autoportal, it will portal if your He/Hr drops by this amount of % lower than your best for current run in dailies, default is 0% (ie: set to 5 to portal at 95% of your best in dailies). Now with stuck protection - Allows portaling midzone if we exceed set buffer amount by 5x. (ie a normal 2% buffer setting would now portal mid-zone you fall below 10% buffer).', 'value', '0', null, 'Daily', [1, 2],
			function () { return (getPageSetting('dailyPortal', currSettingUniverse) === 1) });
		...
		//Radon Daily Portal
		createSetting('dailyPortalFiller', 'Filler run', 'Will automatically run a filler (challenge selected in DP: Challenge) if you\'re already in a daily and have this enabled.', 'boolean', false, null, 'Daily', [1, 2],
			function () { return (getPageSetting('dailyPortal', currSettingUniverse) > 0) });

		createSetting('dailyPortalPreviousUniverse', 'Daily prev universe', 'If this is on, you will do your daily in U2. ', 'boolean', false, null, 'Daily', [2],
	...
	if (displayC2) {
		//Helium
		createSetting('c2Finish', 'Finish C2', '<b>DONT USE THIS WITH C2 RUNNER. ITS FINISH ZONE WILL OVERRIDE THIS</b><br>Finish / Abandon Challenge2 (any) when this zone is reached, if you are running one. For manual use. Recommended: Zones ending with 0 for most Challenge2. Disable with -1. Does not affect Non-Challenge2 runs.', 'value', -1, null, 'C2', [1, 2]);
		createSetting('c2Table', 'C2 Table', 'Display your challenge runs in a convenient table which is colour coded. <br><b>Green</b> = Not worth updating. <br><b>Yellow</b> = Consider updating. <br><b>Red</b> = Updating this challenge is worthwhile. <br><b>Blue</b> = You have not yet done this challenge. ', 'infoclick', 'c2table', null, 'C2', [1, 2]);
		...
		createSetting('c3buildingzone', 'Buy buildings till', 'When in a C3 or special challenge  (Mayhem, Panda) will spend 99% of resource on buildings until this zone.', 'value', -1, null, 'C2', [2],
			function () { return (!autoBattle.oneTimers.Expanding_Tauntimp.owned) });
		createSetting('c2SharpTrimps', 'C2 Sharp Trimps', 'Will purchase Sharp Trimps during C2 or special challenge (Mayhem, Pandemonium) runs when enabled.', 'boolean', false, null, 'C2', [1, 2]);
		createSetting('c2GoldenMaps', 'C2 Golden Maps', 'Will purchase Golden Maps during C2 or special challenge (Mayhem, Pandemonium) runs when enabled.', 'boolean', false, null, 'C2', [1, 2]);
		//C2 Runner
		createSetting('c2RunnerStart', 'C2 Runner', 'Runs the normal C2s in sequence according to difficulty. See \'C2 Table\' for a list of challenges that this can run. Once zone you have defined has been reached, will portal into next. Only runs challenges that need updating, will not run ones close-ish to your HZE. ', 'boolean', false, null, 'C2', [1, 2]);
		createSetting('c2RunnerPortal', 'C2 Runner Portal', 'Automatically portal when this level is reached in C2 Runner. Set to 0 or -1 to disable.', 'value', '-1', null, 'C2', [1, 2],
		...
		//Unbalance
		createSetting('unbalance', 'Unbalance', 'Turn this on if you want to enable Unbalance destacking feautres.', 'boolean', false, null, 'C2', [2]);
		createSetting('unbalanceZone', 'U: Zone', 'Which zone you would like to start destacking from.', 'value', [6], null, 'C2', [2],
			function () { return (getPageSetting('unbalance', currSettingUniverse)) });
		createSetting('unbalanceStacks', 'U: Stacks', 'The amount of stack you have to reach before clearing them.', 'value', -1, null, 'C2', [2],
			function () { return (getPageSetting('unbalance', currSettingUniverse)) });
		createSetting('unbalanceImprobDestack', 'U: Improbability Destack', 'Turn this on to always go down to 0 Balance on Improbabilities after you reach your specified destacking zone', 'boolean', false, null, 'C2', [2],
			function () { return (getPageSetting('unbalance', currSettingUniverse)) });

		//Trappapalooza
		createSetting('trappapalooza', 'Trappa', 'Turn this on if you want to enable Trappa feautres.', 'boolean', false, null, 'C2', [2],
			function () { return (game.global.highestRadonLevelCleared + 1 >= 60) });
		createSetting('trappapaloozaCoords', 'T: Coords', 'The zone you would like to stop buying additional coordinations at.', 'value', -1, null, 'C2', [2],
			function () { return (getPageSetting('trappapalooza', currSettingUniverse)) });

		//Quest
		createSetting('quest', 'Quest', 'Turn this on if you want AT to automate Quests. Will only function properly with AutoMaps enabled.', 'boolean', true, null, 'C2', [2],
			function () { return (game.global.highestRadonLevelCleared + 1 >= 85) });
		createSetting('questSmithyZone', 'Q: Smithy Zone', 'The zone you\'ll stop your Quest run at (will assume 85 for non C3 version). Will calculate the smithies required for Quests and purchase spare ones if possible.', 'value', [999], null, 'C2', [2],
			function () { return (getPageSetting('quest', currSettingUniverse)) });

		//Mayhem
		createSetting('mayhem', 'Mayhem', 'Turn on Mayhem settings. ', 'boolean', false, null, 'C2', [2],
		...
		createSetting('mayhemDestack', 'M: HD Ratio', 'What HD ratio cut-off to use when farming for the boss. If this setting is 100, the script will destack until you can kill the boss in 100 average hits or there are no Mayhem stacks remaining to clear. ', 'value', '-1', null, 'C2', [2],
			function () { return (getPageSetting('mayhem', currSettingUniverse)) });
		createSetting('mayhemZone', 'M: Zone', 'What zone you\'d like to start destacking from, can be used in conjunction with \'M: HD Ratio\' but will clear stacks until 0 are remaining regardless of the value set in \'M: HD Ratio\'.', 'value', '-1', null, 'C2', [2],
			function () { return (getPageSetting('mayhem', currSettingUniverse)) });
		createSetting('mayhemMapIncrease', 'M: Map Increase', 'Will increase the map level of Mayhem farming by this value for if you find the map level AT is selecting is too low. Negative values will be automatically set to 0.<br>This setting will make it so that AT doesn\'t check if you can afford the new map level so beware it could cause some issues.', 'value', '-1', null, 'C2', [2],
			function () { return (getPageSetting('mayhem', currSettingUniverse)) });

		//Storm
		createSetting('storm', 'Storm', 'Turn on Storm settings. This also controls the entireity of Storm settings. If you turn this off it will not do anything in Storm. ', 'boolean', false, null, 'C2', [2],
			function () { return (game.global.highestRadonLevelCleared + 1 >= 105) });
		createSetting('stormZone', 'S: Zone', 'Which zone you would like to start destacking from.', 'value', [6], null, 'C2', [2],
			function () { return (getPageSetting('storm', currSettingUniverse)) });
		createSetting('stormStacks', 'S: Stacks', 'The amount of stack you have to reach before clearing all of them.', 'value', -1, null, 'C2', [2],
			function () { return (getPageSetting('storm', currSettingUniverse)) });

		//Pandemonium
		createSetting('pandemonium', 'Pandemonium', 'Turn on Pandemonium settings.', 'boolean', false, null, 'C2', [2],
		...
		createSetting('pandemoniumZone', 'P: Destack Zone', 'What zone to start Pandemonium mapping at. Will ignore Pandemonium stacks below this zone.', 'value', '-1', null, 'C2', [2],
			function () { return (getPageSetting('pandemonium', currSettingUniverse)) });

		createSetting('pandemoniumAE', ['P: AutoEquip Off', 'P: AutoEquip', 'P AE: LMC', 'P AE: Huge Cache'], '<b>P: AutoEquip</b><br>Will automatically purchase equipment during Pandemonium regardless of efficiency.<br><br/><b>P AE: LMC Cache</b><br>Provides settings to run maps if the cost of equipment levels is less than a single large metal cache<br/>Will also purchase prestiges when they cost less than a Jestimp proc. Additionally will override worker settings to ensure that you farm as much metal as possible.<br/><br><b>P AE: Huge Cache</b><br>Uses the same settings as \'P: AE LMC\' but changes to if an equip will cost less than a single huge cache that procs metal. Will automatically switch caches between LMC and HC depending on the cost of equipment to ensure fast farming speed.', 'multitoggle', 0, null, 'C2', [2],
			function () { return (getPageSetting('pandemonium', currSettingUniverse)) });

		createSetting('pandemoniumAEZone', 'P AE: Zone', 'Which zone you would like to start farming as much gear as possible from.', 'value', '-1', null, 'C2', [2],
		...
		createSetting('pandemoniumStaff', 'P: Staff', 'The name of the staff you would like to equip while equip farming, should ideally be a full metal efficiency staff.', 'textValue', 'undefined', null, 'C2', [2],
			function () { return (getPageSetting('pandemonium', currSettingUniverse) && getPageSetting('pandemoniumAE', currSettingUniverse) > 1) });

		createSetting('pandemoniumMP', 'P: Melting Point', 'How many smithies to run Melting Point at during Pandemonium. <b>THIS OVERRIDES UNIQUE MAP SETTINGS INPUTS</b>', 'value', '-1', null, 'C2', [2],
			function () { return (getPageSetting('pandemonium', currSettingUniverse)) });

		//Glass
		createSetting('glass', 'Glass', 'Turn this on if you want to enable automating Glass damage farming & destacking feautres.', 'boolean', false, null, 'C2', [2],
			function () { return (game.global.highestRadonLevelCleared + 1 >= 175) });
		createSetting('glassStacks', 'G: Stacks', 'The amount of stack you have to reach before clearing them.', 'value', -1, null, 'C2', [2],
			function () { return (getPageSetting('glass', currSettingUniverse)) });

		//Smithless
		createSetting('smithless', 'Smithless', 'Turn this on if you want to enable AT farming for damage to kill Ubersmiths on the Smithless challenge. It will identify breakpoints that can be reached with max tenacity & max map bonus to figure out how many stacks you are able to obtain from the Ubersmith on your current zone and farm till it reached that point if it\'s attainable.', 'boolean', false, null, 'C2', [2],
	...
	if (displayChallenges) {
		...
		createSetting('decayStacksToPush', 'D: Stacks to Push', 'During Decay, AT will ignore maps and push to end the zone if we go above this amount of stacks.<br><br>Use -1 or 0 to disable.<br>Defaults to 300.', 'value', '300', null, 'Challenges', [1],
		...
		createSetting('archaeologyString1', 'First String', 'First string to use in Archaeology. Put the zone you want to stop using this string and start using the second string (Make sure the second string has a value) at the start. I.e: 70,10a,10e ', 'textValue', 'undefined', null, 'Challenges', [2],
			function () { return (autoTrimpSettings.archaeology.enabledU2) });
		createSetting('archaeologyString2', 'Second String', 'Second string to use in Archaeology. Put the zone you want to stop using this string and start using the third string (Make sure the third string has a value) at the start. I.e: 94,10a,10e ', 'textValue', 'undefined', null, 'Challenges', [2],
			function () { return (autoTrimpSettings.archaeology.enabledU2) });
		createSetting('archaeologyString3', 'Third String', 'Third string to use in Archaeology. Make sure this is just your Archaeology string and nothing else. I.e: 10a,10e ', 'textValue', 'undefined', null, 'Challenges', [2],
			function () { return (autoTrimpSettings.archaeology.enabledU2) });
	...
	if (displayBuildings) {
		createSetting('buildingsType', 'AutoBuildings', 'Buys buildings in an efficient way. Also enables Vanilla AutoStorage if its off. ', 'boolean', 'true', null, 'Legacy', [1]);
		createSetting('buildingSettingsArray', 'Building Settings', 'Click to adjust settings. ', 'mazDefaultArray', {
	...
	if (displayGear) {
		createSetting('equipOn', 'AutoEquip', 'AutoEquip. Buys Prestiges and levels equipment according to various settings. Will only buy prestiges if it is worth it. Levels all eqiupment according to best efficiency.', 'boolean', false, null, "Gear", [1, 2]);
		createSetting('equipCutOff', 'AE: Cut-off', 'Decides when to buy gear. 1 is default. This means it will take 1 hit to kill an enemy. If zone is below the zone you have defined in AE: Zone then it will only buy equips when needed.', 'value', '1', null, 'Gear', [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipAmount', 'AE: Amount', 'How much equipment to level per time.', 'value', 1, null, "Gear", [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipCapAttack', 'AE: Weapon Cap', 'What level to stop buying Weapons at.', 'value', 50, null, "Gear", [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipCapHealth', 'AE: Armour Cap', 'What level to stop buying Armour at.', 'value', 50, null, "Gear", [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipZone', 'AE: Zone', 'What zone to stop caring about H:D and buy as much prestiges and equipment as possible. <br><br>Can input multiple zones such as \'200\,231\,251\', doing this will spend all your resources purchasing gear and prestiges on each zone input but will only buy them until the end of the run after the last input. ', 'multiValue', -1, null, "Gear", [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipPercent', 'AE: Percent', 'What percent of resources to spend on equipment before the zone you have set in AE: Zone.', 'value', 1, null, "Gear", [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipPortal', 'AE: Portal', 'Makes sure Auto Equip is on after portalling. Turn this off to disable this and remember your choice.', 'boolean', false, null, 'Gear', [1, 2]);
		createSetting('equip2', 'AE: 2', 'Always buys level 2 of weapons and armor regardless of efficiency.', 'boolean', true, null, "Gear", [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipPrestige', ['AE: Prestige Off', 'AE: Prestige', 'AE: Always Prestige'], '\
	...
	<b>AE: Always Prestige</b><br>Always buys prestiges of weapons and armor regardless of efficiency. Will override AE: Zone setting for an equip if it has a prestige available.', 'multitoggle', 0, null, "Gear", [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipHighestPrestige', 'AE: Highest Prestige', 'Will only buy equips for the highest prestige currently owned.', 'boolean', true, null, "Gear", [1, 2],
			function () { return (getPageSetting('equipOn', currSettingUniverse) && getPageSetting('equipPrestige', currSettingUniverse) !== 0) });
		createSetting('equipEfficientEquipDisplay', 'AE: Highlight Equips', 'Will highlight the most efficient equipment or prestige to buy. <b>This setting will disable the default game setting.', 'boolean', true, null, "Gear", [1, 2]);
		createSetting('equipShieldBlock', 'Buy Shield Block', 'Will buy the shield block upgrade. CAUTION: If you are progressing past zone 60, you probably don\'t want this :)', 'boolean', false, null, "Gear", [1],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });
		createSetting('equipNoShields', 'AE: No Shields', 'Will stop AT from buying Shield prestiges or upgrades when they\'re available.', 'boolean', false, null, "Gear", [2],
			function () { return (getPageSetting('equipOn', currSettingUniverse)) });

		createSetting('Prestige', 'Prestige', 'Acquire prestiges through the selected item (inclusive) as soon as they are available in maps. Forces equip first mode. Automap must be enabled. THIS IS AN IMPORTANT SETTING related to speed climbing and should probably always be on something. If you find the script getting stuck somewhere, particularly where you should easily be able to kill stuff, setting this to an option lower down in the list will help ensure you are more powerful at all times, but will spend more time acquiring the prestiges in maps.', 'dropdown', 'Off', ['Off', 'Supershield', 'Dagadder', 'Bootboost', 'Megamace', 'Hellishmet', 'Polierarm', 'Pantastic', 'Axeidic', 'Smoldershoulder', 'Greatersword', 'Bestplate', 'Harmbalest', 'GambesOP'], "Gear", [1, 2]);
		createSetting('ForcePresZ', 'Force Prestige Z', 'On and after this zone is reached, always try to prestige for everything immediately, ignoring Dynamic Prestige settings and overriding that of Linear Prestige. Prestige Skip mode will exit this. Disable with -1.', 'value', -1, null, 'Gear', [1, 2]);
		createSetting('PrestigeSkip1_2', ['Prestige Skip Off', 'Prestige Skip 1 & 2', 'Prestige Skip 1', 'Prestige Skip 2'], '<b>Prestige Skip 1:</b> If there are more than 2 Unbought Prestiges (besides Shield), ie: sitting in your upgrades window but you cant afford them, AutoMaps will not enter Prestige Mode, and/or will exit from it. The amount of unboughts can be configured with this variable MODULES[\\"maps\\"].SkipNumUnboughtPrestiges = 2; <br><b>Prestige Skip 2:</b> If there are 2 or fewer <b>Unobtained Weapon Prestiges in maps</b>, ie: there are less than 2 types to run for, AutoMaps will not enter Prestige Mode, and/or will exit from it. For users who tends to not need the last few prestiges due to resource gain not keeping up. The amount of unboughts can be configured with MODULES.maps.UnearnedPrestigesRequired. If PrestigeSkipMode is enabled, both conditions need to be reached before exiting.', 'multitoggle', 0, null, "Gear", [1, 2]);
	}
	...
	if (displayMaps) {
		//Helium
		createSetting('autoMaps', ["Auto Maps Off", "Auto Maps On", "Auto Maps No Unique"], 'Automaps. The no unique setting will not run unique maps such as dimensions of anger. Recommended ON. Do not use window, it will not work. ', 'multitoggle', 0, null, "Maps", [1, 2]);
		...
		//HD Farm
		createSetting('hdFarmPopup', 'HD Farm Settings', 'Click to adjust settings. Not fully implemented yet, still need to add in an Atlantrimp setting.', 'action', 'MAZLookalike("HD Farm", "HDFarm", "MAZ")', null, 'Maps', [1, 2]);
		createSetting('hdFarmSettings', 'HD Farm: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [1, 2]);
		createSetting('hdFarmDefaultSettings', 'HD Farm: Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [1, 2]);

		//Void Maps
		createSetting('voidMapPopup', 'Void Map Settings', 'Will run all of your Void Maps on a specified zone according to this settings value.', 'action', 'MAZLookalike("Void Map", "VoidMap", "MAZ")', null, 'Maps', [1, 2]);
		createSetting('voidMapSettings', 'Void Map Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [1, 2]);
		createSetting('voidMapDefaultSettings', 'Void Map Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [1, 2]);

		//Bone Shrine (bone) 
		createSetting('boneShrinePopup', 'Bone Shrine Settings', 'Will use a specified amount of Bone Shrine charges according to this settings value.', 'action', 'MAZLookalike("Bone Shrine", "BoneShrine", "MAZ")', null, 'Maps', [1, 2]);
		createSetting('boneShrineSettings', 'BS: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [1, 2]);
		createSetting('boneShrineDefaultSettings', 'BS: Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [1, 2]);

		//Worshipper Farm 
		createSetting('worshipperFarmPopup', 'Worshipper Farm Settings', 'Will farm to a specified amount of Worshippers according to this settings value.', 'action', 'MAZLookalike("Worshipper Farm", "WorshipperFarm", "MAZ")', null, 'Maps', [2],
			function () { return game.global.highestRadonLevelCleared + 1 >= 50 });
		createSetting('worshipperFarmSettings', 'WF: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [2]);
		createSetting('worshipperFarmDefaultSettings', 'WF: Default Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [2]);

		//Map Bonus
		createSetting('mapBonusPopup', 'Map Bonus Settings', 'Will map stack to a specified amount according to this settings value.', 'action', 'MAZLookalike("Map Bonus", "MapBonus", "MAZ")', null, 'Maps', [1, 2]);
		createSetting('mapBonusSettings', 'Map Bonus: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [1, 2]);
		createSetting('mapBonusDefaultSettings', 'Map Bonus: Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [1, 2]);
		createSetting('mapBonusZone', 'Map Bonus: Zone', 'Map Bonus', 'multiValue', [6], null, 'Legacy', [1, 2]);

		//Map Farm
		createSetting('mapFarmPopup', 'Map Farm Settings', 'Will farm a specified amount of maps according to this settings value.', 'action', 'MAZLookalike("Map Farm", "MapFarm", "MAZ")', null, 'Maps', [1, 2]);
		createSetting('mapFarmSettings', 'MF: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [1, 2]);
		createSetting('mapFarmDefaultSettings', 'MF: Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [1, 2]);

		//Prestige Raiding
		createSetting('raidingPopup', 'Raiding Settings', 'Will raid up to a specified zone according to this settings value.', 'action', 'MAZLookalike("Raiding", "Raiding", "MAZ")', null, 'Maps', [1, 2]);
		createSetting('raidingSettings', 'Raiding: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [1, 2]);
		createSetting('raidingDefaultSettings', 'Raiding: Default Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [1, 2]);

		//Bionic Raiding
		createSetting('bionicRaidingPopup', 'BW Raiding Settings', 'Will Bionic Wonderlands up to a specified zone according to this settings value.', 'action', 'MAZLookalike("Bionic Raiding", "BionicRaiding", "MAZ")', null, 'Maps', [1]);
		createSetting('bionicRaidingSettings', 'Raiding: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [1]);
		createSetting('bionicRaidingDefaultSettings', 'Raiding: Default Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [1]);

		//Tribute Farming
		createSetting('tributeFarmPopup', 'Tribute Farm Settings', 'Will farm for a specified amount of Tributes/Meteorologists according to this settings value.', 'action', 'MAZLookalike("Tribute Farm", "TributeFarm", "MAZ")', null, 'Maps', [2]);
		createSetting('tributeFarmSettings', 'TrF: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [2]);
		createSetting('tributeFarmDefaultSettings', 'TrF: Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [2]);

		//Smithy Farming
		createSetting('smithyFarmPopup', 'Smithy Farm Settings', 'Will farm for a specified amount of Smithies according to this settings value.', 'action', 'MAZLookalike("Smithy Farm", "SmithyFarm", "MAZ")', null, 'Maps', [2]);
		createSetting('smithyFarmSettings', 'SF: Settings', 'Contains arrays for this setting', 'mazArray', [], null, 'Maps', [2]);
		createSetting('smithyFarmDefaultSettings', 'SF: Settings', 'Contains arrays for this setting', 'mazDefaultArray', { active: false }, null, 'Maps', [2]);
	...
	if (displayCombat) {
		//Helium
		createSetting('autoFight', ['Better AutoFight OFF', 'Better Auto Fight', 'Vanilla'], '3-Way Button, Recommended. Will automatically handle fighting.<br>BAF = Old Algo (Fights if dead, new squad ready, new squad breed timer target exceeded, and if breeding takes under 0.5 seconds<br>BAF3 = Uses vanilla autofight and makes sure you fight on portal. <br> WARNING: If you autoportal with BetterAutoFight disabled, the game may sit there doing nothing until you click FIGHT. (not good for afk) ', 'multitoggle', 1, null, "Combat", [1, 2]);
		...
		//Radon
		createSetting('equalityManagement', ['Auto Equality Off', 'Auto Equality: Basic', 'Auto Equality: Advanced'], 'Manages Equality settings for you. <br><br><b>Auto Equality: Basic</b><br>Sets Equality to 0 on Slow enemies, and Autoscaling on for Fast enemies.<br><br><b>Auto Equality: Advanced</b><br>Will automatically identify the best equality levels to kill the current enemy and change it when necessary.', 'multitoggle', 0, null, 'Combat', [2]);
		createSetting('equalityCalc', ['Equality Calc Off', 'EC: On', 'EC: Health'], '<b>Experimental. </b><br>Adds Equality Scaling levels to the battlecalc. Will always calculate equality based on actual scaling levels when its turned off by other settings. Assumes you use Equality Scaling. Turning this on allows in-game Equality Scaling to adjust your Health accordingly. EC: Health only decreases enemies attack in the calculation which may improve speed. ', 'multitoggle', 0, null, 'Combat', [2],
			function () { return (getPageSetting('equalityManagement') < 2) });
		createSetting('gammaBurstCalc', 'Gamma Burst Calc', '<b>Experimental.</b><br>Adds Gamma Burst to your HD Ratio. Be warned, it will assume that you have a gamma burst ready to trigger for every attack so your HD Ratio might be 1 but you\'d need to attack 4-5 times to reach that damage theshold.', 'boolean', true, null, 'Combat', [1, 2],
			function () { return (getPageSetting('equalityManagement') === 2) });
		createSetting('frenzyCalc', 'Frenzy Calc', '<b>Experimental.</b><br>Adds frenzy to the calc. Be warned, it will not farm as much with this on as it expects 100% frenzy uptime.', 'boolean', false, null, 'Combat', [2],
	...
	if (displayHeirlooms) {
		...
		createSetting('heirloomAuto', 'Auto Heirlooms', 'Auto Heirlooms master button. Turn this on to enable all Auto Heirloom settings. <br><br><b>The Modifier points will be explained here.</b> The more points an heirloom has, the better chance it has of being kept. If empty is selected, it will muliplty the score by 4. If any is selected, it will multiply the score of the heirloom by 2. <br><br>E.g Mod 1 = CC (+6 if dropped, 1st modifier) <br>Mod 2 = CD (+5 if dropped, 2nd modifier) <br>Mod 3 = PB (+4 if dropped, 3rd modifier) <br>Mod 4 = Empty (x4 if dropped, +0 if not) <br>Mod 5 = Empty (x4 if dropped, +0 if not) <br><br>If an heirloom dropped with these exact modifiers, it would get a score of 192 (6+5+4*4*4=240). The highest point heirlooms will be kept. ', 'boolean', false, null, 'Heirlooms', [1, 2]);

		createSetting('heirloomAutoTypeToKeep', ['None', 'Shields', 'Staffs', 'Cores', 'All'], '<b>Shields: </b>Keeps Shields and nothing else.<br><b>Staffs: </b>Keeps Staffs and nothing else.<br><b>Cores: </b>Keeps Cores and nothing else.<br><b>All: </b>Keeps 4 Shields and 3 Staffs and 3 Cores. If you have protected heirlooms in your inventory it will overrite one slot. E.g if one heirloom is protected, you will keep 4 Shields and 3 Staffs and 2 Cores. ', 'multitoggle', 0, null, 'Heirlooms', [1, 2],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse)) });
		createSetting('heirloomAutoRareToKeep', 'Rarity to Keep', 'Auto Heirlooms. Keeps the selected rarity of heirloom, recycles all others. ', 'dropdown', 'Any', ["None", "Common", "Uncommon", "Rare", "Epic", "Legendary", "Magnificent", "Ethereal", "Magmatic", "Plagued", "Radiating", "Hazardous", "Enigmatic"], 'Heirlooms', [1, 2],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse)) });
		createSetting('heirloomAutoOnlyPefect', 'Only Perfect', 'Will make sure that Auto Heirlooms will only keep heirlooms that have the mods you selected on them. Be warned you have to ensure that all modifier slots have been selected when using this setting or it won\'t function properly.', 'boolean', false, null, 'Heirlooms', [1, 2],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse)) });

		//Shield Line
		createSetting('heirloomAutoShield', 'Shields', 'Auto Heirlooms. Enables in-depth shield settings. ', 'boolean', false, null, 'Heirlooms', [1, 2],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse)) });

		createSetting('heirloomAutoShieldMod1', 'Shield: Modifier 1', 'Auto Heirlooms. Keeps Shields with selected Mod. Modifier 1 is worth 6 points. ', 'dropdown', 'empty', ["empty", "playerEfficiency", "trainerEfficiency", "storageSize", "breedSpeed", "trimpHealth", "trimpAttack", "trimpBlock", "critDamage", "critChance", "voidMaps", "plaguebringer", "prismatic", "gammaBurst", "inequality"], 'Heirlooms', [1, 2],
			function () {
		...
		createSetting('heirloomAutoShieldMod2', 'Shield: Modifier 2', 'Auto Heirlooms. Keeps Shields with selected Mod. Modifier 2 is worth 5 points. ', 'dropdown', 'empty', ["empty", "playerEfficiency", "trainerEfficiency", "storageSize", "breedSpeed", "trimpHealth", "trimpAttack", "trimpBlock", "critDamage", "critChance", "voidMaps", "plaguebringer", "prismatic", "gammaBurst", "inequality"], 'Heirlooms', [1, 2],
			function () {
				const heirloomType = ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Magnificent', 'Ethereal', 'Magmatic', 'Plagued', 'Radiating', 'Hazardous', 'Enigmatic'];
		...
		createSetting('heirloomAutoShieldMod3', 'Shield: Modifier 3', 'Auto Heirlooms. Keeps Shields with selected Mod. Modifier 3 is worth 4 points. ', 'dropdown', 'empty', ["empty", "playerEfficiency", "trainerEfficiency", "storageSize", "breedSpeed", "trimpHealth", "trimpAttack", "trimpBlock", "critDamage", "critChance", "voidMaps", "plaguebringer", "prismatic", "gammaBurst", "inequality"], 'Heirlooms', [1, 2],
			function () {
				const heirloomType = ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Magnificent', 'Ethereal', 'Magmatic', 'Plagued', 'Radiating', 'Hazardous', 'Enigmatic'];
		...
		createSetting('heirloomAutoShieldMod5', 'Shield: Modifier 5', 'Auto Heirlooms. Keeps Shields with selected Mod. Modifier 5 is worth 2 points. ', 'dropdown', 'empty', ["empty", "playerEfficiency", "trainerEfficiency", "storageSize", "breedSpeed", "trimpHealth", "trimpAttack", "trimpBlock", "critDamage", "critChance", "voidMaps", "plaguebringer", "prismatic", "gammaBurst", "inequality"], 'Heirlooms', [1, 2],
			function () {
				const heirloomType = ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Magnificent', 'Ethereal', 'Magmatic', 'Plagued', 'Radiating', 'Hazardous', 'Enigmatic'];
		...
		createSetting('heirloomAutoShieldMod6', 'Shield: Modifier 6', 'Auto Heirlooms. Keeps Shields with selected Mod. Modifier 6 is worth 1 points. ', 'dropdown', 'empty', ["empty", "playerEfficiency", "trainerEfficiency", "storageSize", "breedSpeed", "trimpHealth", "trimpAttack", "trimpBlock", "critDamage", "critChance", "voidMaps", "plaguebringer", "prismatic", "gammaBurst", "inequality"], 'Heirlooms', [1, 2],
			function () {
				const heirloomType = ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Magnificent', 'Ethereal', 'Magmatic', 'Plagued', 'Radiating', 'Hazardous', 'Enigmatic'];
		...
		createSetting('heirloomAutoShieldMod7', 'Shield: Modifier 7', 'Auto Heirlooms. Keeps Shields with selected Mod. Modifier 7 is worth 1 points. ', 'dropdown', 'empty', ["empty", "playerEfficiency", "trainerEfficiency", "storageSize", "breedSpeed", "trimpHealth", "trimpAttack", "trimpBlock", "critDamage", "critChance", "voidMaps", "plaguebringer", "prismatic", "gammaBurst", "inequality"], 'Heirlooms', [1, 2],
			function () {
				const heirloomType = ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Magnificent', 'Ethereal', 'Magmatic', 'Plagued', 'Radiating', 'Hazardous', 'Enigmatic'];
		...
		//Staff Line
		createSetting('heirloomAutoStaff', 'Staffs', 'Auto Heirlooms. Enables in-depth staff settings. ', 'boolean', false, null, 'Heirlooms', [1, 2],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse)) });

		createSetting('heirloomAutoStaffMod1', 'Staff: Modifier 1', 'Auto Heirlooms. Keeps Staffs with selected Mod. Modifier 1 is worth 6 points. ', 'dropdown', 'empty', ["empty", "metalDrop", "foodDrop", "woodDrop", "gemsDrop", "fragmentsDrop", "MinerSpeed", "FarmerSpeed", "LumberjackSpeed", "DragimpSpeed", "ExplorerSpeed", "ScientistSpeed", "FluffyExp", "ParityPower"], 'Heirlooms', [1, 2],
			function () {
				const heirloomType = ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Magnificent', 'Ethereal', 'Magmatic', 'Plagued', 'Radiating', 'Hazardous', 'Enigmatic'];
		...
		//Core Line
		createSetting('heirloomAutoCore', 'Cores', 'Auto Heirlooms. Enables in-depth core settings. ', 'boolean', false, null, 'Heirlooms', [1],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse)) });
		createSetting('heirloomAutoCoreMod1', 'Cores: Modifier 1', 'Auto Heirlooms. Keeps Cores with selected Mod. Modifier 1 is worth 5 points. ', 'dropdown', 'empty', ["empty", "fireTrap", "poisonTrap", "lightningTrap", "runestones", "strengthEffect", "condenserEffect"], 'Heirlooms', [1],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse) && getPageSetting('heirloomAutoCore', currSettingUniverse)) });
		createSetting('heirloomAutoCoreMod2', 'Cores: Modifier 2', 'Auto Heirlooms. Keeps Cores with selected Mod. Modifier 2 is worth 4 points. ', 'dropdown', 'empty', ["empty", "fireTrap", "poisonTrap", "lightningTrap", "runestones", "strengthEffect", "condenserEffect"], 'Heirlooms', [1],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse) && getPageSetting('heirloomAutoCore', currSettingUniverse)) });
		createSetting('heirloomAutoCoreMod3', 'Cores: Modifier 3', 'Auto Heirlooms. Keeps Cores with selected Mod. Modifier 3 is worth 3 points. ', 'dropdown', 'empty', ["empty", "fireTrap", "poisonTrap", "lightningTrap", "runestones", "strengthEffect", "condenserEffect"], 'Heirlooms', [1],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse) && getPageSetting('heirloomAutoCore', currSettingUniverse)) });
		createSetting('heirloomAutoCoreMod4', 'Cores: Modifier 4', 'Auto Heirlooms. Keeps Cores with selected Mod. Modifier 4 is worth 2 points. ', 'dropdown', 'empty', ["empty", "fireTrap", "poisonTrap", "lightningTrap", "runestones", "strengthEffect", "condenserEffect"], 'Heirlooms', [1],
			function () { return (getPageSetting('heirloomAuto', currSettingUniverse) && getPageSetting('heirloomAutoCore', currSettingUniverse)) });
	}
	...
	if (displayGolden) {
		//Helium
		createSetting('autoGoldenPopup', 'Auto Gold Settings', 'Click to adjust settings. ', 'action', 'MAZLookalike("Auto Golden", "AutoGolden", "MAZ")', null, 'Golden', [1, 2]);
		createSetting('autoGoldenSettings', 'AutoGoldenUpgrades', 'Contains arrays for this setting', 'mazArray', [], null, 'Golden', [1, 2]);
		createSetting('autoGoldenDailyPopup', 'Daily Auto Gold Settings', 'Click to adjust settings. ', 'action', 'MAZLookalike("Daily Auto Golden", "AutoGoldenDaily", "MAZ")', null, 'Golden', [1, 2]);
		createSetting('autoGoldenDailySettings', 'Daily AutoGoldenUpgrades', 'Contains arrays for this setting', 'mazArray', [], null, 'Golden', [1, 2]);
		createSetting('autoGoldenC3Popup', 'C3 Auto Gold Settings', 'Click to adjust settings. ', 'action', 'MAZLookalike("C3 Auto Golden", "AutoGoldenC3", "MAZ")', null, 'Golden', [1, 2]);
		createSetting('autoGoldenC3Settings', 'C3 AutoGoldenUpgrades', 'Contains arrays for this setting', 'mazArray', [], null, 'Golden', [1, 2]);
	}
	...
	if (displayDisplay) {
		createSetting('displayEnhancedGrid', 'Enhance Grids', 'Apply slight visual enhancements to world and map grids that highlights with drop shadow all the exotic, powerful, skeletimps and other special imps.', 'boolean', false, null, 'Display', [1, 2]);
		createSetting('displayAutoMapStatus', 'AutoMap Status', 'Enables the display of the map status. Turn this off to reduce memory. ', 'boolean', true, null, 'Display', [1, 2]);
		createSetting('displayHeHr', 'He/hr status', 'Enables the display of your helium per hour. Turn this off to reduce memory. ', 'boolean', true, null, 'Display', [1, 2]);

		createSetting('displayAllSettings', 'Display all settings', 'Will display all of the locked settings that have HZE or other requirements to be displayed.', 'boolean', false, null, 'Display', [1, 2]);

		createSetting('EnableAFK', 'Go AFK Mode', '(Action Button). Go AFK uses a Black Screen, and suspends ALL the Trimps GUI visual update functions (updateLabels) to improve performance by not doing unnecessary stuff. This feature is primarily just a CPU and RAM saving mode. Everything will resume when you come back and press the Back button. Console debug output is also disabled. The blue color means this is not a settable setting, just a button. You can now also click the Zone # (World Info) area to go AFK now.', 'action', 'MODULES["performance"].EnableAFKMode()', null, 'Display', [1, 2]);
		...
		createSetting('automateSpireAssault', 'Automate Spire Assault', 'Automates Spire Assault gear swaps from level 92 up to level 128. HIGHLY RECOMMENDED THAT YOU DO NOT USE THIS SETTING.', 'boolean', false, null, 'Display', [0]);
	...
	if (displaySpam) {
		...
		createSetting('sitInMaps', 'Sit in maps', 'Will cause AT to go sit in the map chamber when enabled. The \'Sit In Zone\' setting must be setup for this to work properly.', 'boolean', false, null, 'Display', [1, 2]);
		createSetting('sitInMaps_Zone', 'SIM: Zone', 'AT will go to the map chamber and stop running any maps at this zone.', 'value', -1, null, 'Display', [1, 2],
			function () { return (getPageSetting('sitInMaps', currSettingUniverse)) });
		createSetting('sitInMaps_Cell', 'SIM: Cell', 'AT will go to the map chamber and stop running any maps after this cell has been reached.', 'value', -1, null, 'Display', [1, 2],
			function () { return (getPageSetting('sitInMaps', currSettingUniverse)) });
	}
	...
	if (displayImport) {
		createSetting('ImportAutoTrimps', 'Import AutoTrimps', 'Import your AutoTrimps Settings. Asks you to name it as a profile afterwards.', 'infoclick', 'ImportAutoTrimps', null, 'Import Export', [1, 2]);
...
function modifyParentNodeUniverseSwap() {
	...
	//Core
	modifyParentNode_Initial("radonsettings", 'show');
	...
	//Maps
	//Helium Settings
	modifyParentNode_Initial("scryvoidmaps", 'show');
	modifyParentNode_Initial("worshipperFarmPopup", 'show');

	//Gear equipNoShields
	modifyParentNode_Initial("equipNoShields", 'show');

	//Spire
	//None!

	//Combat
	modifyParentNode_Initial("gammaBurstCalc", radonoff);

	//ATGA
	modifyParentNode_Initial("ATGA2timer", radonoff);

	//C2
	modifyParentNode_Initial("c2GoldenMaps", 'show');
	modifyParentNode_Initial("c2Fused", radonoff);
	modifyParentNode_Initial("balanceImprobDestack", radonoff);

	modifyParentNode_Initial("c2RunnerPercent", radonon);
	modifyParentNode_Initial("unbalanceImprobDestack", radonon);
	modifyParentNode_Initial("trappapaloozaCoords", radonon);
	modifyParentNode_Initial("questSmithyZone", radonon);
	modifyParentNode_Initial("mayhemMapIncrease", radonon_mayhem);
	...
	modifyParentNode_Initial("archaeologyString3", radonon);
	...
	if (getPageSetting('radonsettings') === 0) {
		modifyParentNode_Initial("heirloomAutoOnlyPefect", radonoff_heirloom);
		modifyParentNode_Initial("heirloomAutoShieldMod7", radonoff_heirloom);
		modifyParentNode_Initial("heirloomAutoStaffMod7", radonoff_heirloom);
	}
	//Radon Settings
	if (getPageSetting('radonsettings') === 1) {
		modifyParentNode_Initial("heirloomAutoOnlyPefect", radonon_heirloom);
		modifyParentNode_Initial("heirloomAutoShieldMod7", radonon_heirloom);
	}
	//Golden Upgrades
	//Helium Settings
	//Radon Settings
	...
	modifyParentNode_Initial("automateSpireAssault", radonon);
	modifyParentNode_Initial("showbreedtimer", radonoff);
...
function updateATVersion() {
	...
	if (autoTrimpSettings["ATversion"] !== undefined && autoTrimpSettings["ATversion"].includes('SadAugust')
		...
		if (autoTrimpSettings["ATversion"].split('v')[1] < '6.00') {

			var tempSettings = JSON.parse(localStorage.getItem('autoTrimpSettings'));

			//Core
			const displayCoreChanges = true; //Done
			if (displayCoreChanges) {
				autoTrimpSettings['gatherType'].value = tempSettings.ManualGather2;
				autoTrimpSettings['gatherType'].valueU2 = tempSettings.RManualGather2;

				autoTrimpSettings['upgradeType'].value = tempSettings.BuyUpgradesNew;
				autoTrimpSettings['upgradeType'].valueU2 = tempSettings.RBuyUpgradesNew;
				...
				autoTrimpSettings['presetSwap'].enabledU2 = tempSettings.RPerkSwapping;

				autoTrimpSettings['autoPortal'].selected = tempSettings.AutoPortal;
				autoTrimpSettings['autoPortal'].selectedU2 = tempSettings.RAutoPortal;

				autoTrimpSettings['heliumHourChallenge'].selected = tempSettings.HeliumHourChallenge;
				autoTrimpSettings['heliumHourChallenge'].selectedU2 = tempSettings.RadonHourChallenge;

				autoTrimpSettings['heliumC2Challenge'].selected = tempSettings.HeliumC2Challenge;
				autoTrimpSettings['heliumC2Challenge'].selectedU2 = tempSettings.RadonC3Challenge;

				autoTrimpSettings['autoPortalZone'].value = tempSettings.CustomAutoPortal;
				autoTrimpSettings['autoPortalZone'].valueU2 = tempSettings.RCustomAutoPortal;

				autoTrimpSettings['HeHrDontPortalBefore'].valueU2 = tempSettings.RnHrDontPortalBefore;

				autoTrimpSettings['HeliumHrBuffer'].valueU2 = tempSettings.RadonHrBuffer;
			}

			//Maps
			const displayMapChanges = true; //Done
			if (displayMapChanges) {

				autoTrimpSettings['autoMaps'].value = tempSettings.AutoMaps;
				autoTrimpSettings['autoMaps'].valueU2 = tempSettings.RAutoMaps;
				autoTrimpSettings['autoMapsPortal'].value = tempSettings.automapsportal;
				autoTrimpSettings['autoMapsPortal'].valueU2 = tempSettings.Rautomapsportal;
				autoTrimpSettings['onlyPerfectMaps'].value = tempSettings.onlyPerfectMaps;
				autoTrimpSettings['onlyPerfectMaps'].valueU2 = tempSettings.ronlyPerfectMaps;

				autoTrimpSettings['uniqueMapSettingsArray'].value = tempSettings.hUniqueMapSettingsArray.value;
				autoTrimpSettings['uniqueMapSettingsArray'].valueU2 = tempSettings.rUniqueMapSettingsArray.value;

				autoTrimpSettings['hdFarmSettings'].value = tempSettings.hHDFarmSettings.value;
				autoTrimpSettings['hdFarmSettings'].valueU2 = tempSettings.rHDFarmSettings.value;
				autoTrimpSettings['hdFarmDefaultSettings'].value = tempSettings.hHDFarmDefaultSettings.value;
				autoTrimpSettings['hdFarmDefaultSettings'].valueU2 = tempSettings.rHDFarmDefaultSettings.value;

				autoTrimpSettings['voidMapSettings'].value = tempSettings.hVoidMapSettings.value;
				autoTrimpSettings['voidMapSettings'].valueU2 = tempSettings.rVoidMapSettings.value;
				autoTrimpSettings['voidMapDefaultSettings'].value = tempSettings.hVoidMapDefaultSettings.value;
				autoTrimpSettings['voidMapDefaultSettings'].valueU2 = tempSettings.rVoidMapDefaultSettings.value;

				autoTrimpSettings['boneShrineSettings'].value = tempSettings.hBoneShrineSettings.value;
				autoTrimpSettings['boneShrineSettings'].valueU2 = tempSettings.rBoneShrineSettings.value;
				autoTrimpSettings['boneShrineDefaultSettings'].value = tempSettings.hBoneShrineDefaultSettings.value;
				autoTrimpSettings['boneShrineDefaultSettings'].valueU2 = tempSettings.rBoneShrineDefaultSettings.value;

				autoTrimpSettings['mapBonusSettings'].value = tempSettings.hMapBonusSettings.value;
				autoTrimpSettings['mapBonusSettings'].valueU2 = tempSettings.rMapBonusSettings.value;
				autoTrimpSettings['mapBonusDefaultSettings'].value = tempSettings.hMapBonusDefaultSettings.value;
				autoTrimpSettings['mapBonusDefaultSettings'].valueU2 = tempSettings.rMapBonusDefaultSettings.value;
				autoTrimpSettings['mapBonusZone'].value = tempSettings.hMapBonusZone;
				autoTrimpSettings['mapBonusZone'].valueU2 = tempSettings.rMapBonusZone;

				autoTrimpSettings['mapFarmSettings'].value = tempSettings.hMapFarmSettings.value;
				autoTrimpSettings['mapFarmSettings'].valueU2 = tempSettings.rMapFarmSettings.value;
				autoTrimpSettings['mapFarmDefaultSettings'].value = tempSettings.hMapFarmDefaultSettings.value;
				autoTrimpSettings['mapFarmDefaultSettings'].valueU2 = tempSettings.rMapFarmDefaultSettings.value;

				autoTrimpSettings['raidingSettings'].value = tempSettings.hRaidingSettings.value;
				autoTrimpSettings['raidingSettings'].valueU2 = tempSettings.rRaidingSettings.value;
				autoTrimpSettings['raidingDefaultSettings'].value = tempSettings.hRaidingDefaultSettings.value;
				autoTrimpSettings['raidingDefaultSettings'].valueU2 = tempSettings.rRaidingDefaultSettings.value;

				autoTrimpSettings['bionicRaidingSettings'].value = tempSettings.hBionicRaidingSettings.value;
				autoTrimpSettings['bionicRaidingDefaultSettings'].value = tempSettings.hBionicRaidingDefaultSettings.value;

				autoTrimpSettings['worshipperFarmSettings'].valueU2 = tempSettings.rWorshipperFarmSettings.value;
				autoTrimpSettings['worshipperFarmDefaultSettings'].valueU2 = tempSettings.rWorshipperFarmDefaultSettings.value;

				autoTrimpSettings['tributeFarmSettings'].valueU2 = tempSettings.rTributeFarmSettings.value;
				autoTrimpSettings['tributeFarmDefaultSettings'].valueU2 = tempSettings.rTributeFarmDefaultSettings.value;

				autoTrimpSettings['smithyFarmSettings'].valueU2 = tempSettings.rSmithyFarmSettings.value;
				autoTrimpSettings['smithyFarmDefaultSettings'].valueU2 = tempSettings.rSmithyFarmDefaultSettings.value;
			}

			//Equips
			const displayEquipChanges = true; //Done
			if (displayEquipChanges) {
				autoTrimpSettings['equipOn'].enabled = tempSettings.Hequipon;
				autoTrimpSettings['equipOn'].enabledU2 = tempSettings.Requipon;

				autoTrimpSettings['equipCutOff'].value = tempSettings.Hdmgcuntoff;
				autoTrimpSettings['equipCutOff'].valueU2 = tempSettings.Rdmgcuntoff;

				autoTrimpSettings['equipAmount'].value = tempSettings.Hequipamount;
				autoTrimpSettings['equipAmount'].valueU2 = tempSettings.Requipamount;

				autoTrimpSettings['equipCapAttack'].value = tempSettings.Hequipcapattack;
				autoTrimpSettings['equipCapAttack'].valueU2 = tempSettings.Requipcapattack;

				autoTrimpSettings['equipCapHealth'].value = tempSettings.Hequipcaphealth;
				autoTrimpSettings['equipCapHealth'].valueU2 = tempSettings.Requipcaphealth;

				autoTrimpSettings['equipZone'].value = tempSettings.Hequipzone;
				autoTrimpSettings['equipZone'].valueU2 = tempSettings.Requipzone;

				autoTrimpSettings['equipPercent'].value = tempSettings.Hequippercent;
				autoTrimpSettings['equipPercent'].valueU2 = tempSettings.Requippercent;

				autoTrimpSettings['equipPortal'].enabled = tempSettings.Hautoequipportal;
				autoTrimpSettings['equipPortal'].enabledU2 = tempSettings.Rautoequipportal;

				autoTrimpSettings['equip2'].enabled = tempSettings.Hequip2;
				autoTrimpSettings['equip2'].enabledU2 = tempSettings.Requip2;

				autoTrimpSettings['equipPrestige'].value = tempSettings.Hequipprestige;
				autoTrimpSettings['equipPrestige'].valueU2 = tempSettings.Requipprestige;

				autoTrimpSettings['equipHighestPrestige'].enabled = tempSettings.hEquipHighestPrestige;
				autoTrimpSettings['equipHighestPrestige'].enabledU2 = tempSettings.rEquipHighestPrestige;

				autoTrimpSettings['equipEfficientEquipDisplay'].enabled = tempSettings.hEquipEfficientEquipDisplay;
				autoTrimpSettings['equipEfficientEquipDisplay'].enabledU2 = tempSettings.rEquipEfficientEquipDisplay;

				autoTrimpSettings['equipShieldBlock'].enabled = tempSettings.BuyShieldBlock;
				autoTrimpSettings['equipNoShields'].enabledU2 = tempSettings.rEquipNoShields;

				autoTrimpSettings['Prestige'].selectedU2 = tempSettings.rPrestige;
				autoTrimpSettings['ForcePresZ'].valueU2 = tempSettings.rForcePresZ;
				autoTrimpSettings['PrestigeSkip1_2'].valueU2 = tempSettings.rPrestigeSkip1_2;
			}

			//Combat
			const displayCombatChanges = true; //Done
			if (displayCombatChanges) {
				autoTrimpSettings['autoFight'].value = tempSettings.BetterAutoFight;
				autoTrimpSettings['autoFight'].valueU2 = tempSettings.BetterAutoFight;
				autoTrimpSettings['autoAbandon'].value = tempSettings.PowerSaving;
				autoTrimpSettings['autoAbandon'].valueU2 = tempSettings.PowerSaving;

				autoTrimpSettings['equalityManagement'].valueU2 = tempSettings.rManageEquality;
				autoTrimpSettings['equalityCalc'].valueU2 = tempSettings.Rcalcmaxequality;

				autoTrimpSettings['gammaBurstCalc'].enabledU2 = tempSettings.rCalcGammaBurst;
				autoTrimpSettings['frenzyCalc'].enabledU2 = tempSettings.Rcalcfrenzy;
			}

			//Daily
			const displayDailyChanges = true; //Done 
			if (displayDailyChanges) {

				autoTrimpSettings['dailyPortalStart'].enabled = tempSettings.AutoStartDaily;
				autoTrimpSettings['dailyPortalStart'].enabledU2 = tempSettings.RAutoStartDaily;

				autoTrimpSettings['dailyPortalPreviousUniverse'].enabled = tempSettings.u2daily;
				autoTrimpSettings['dailyPortalPreviousUniverse'].enabledU2 = tempSettings.u1daily;

				autoTrimpSettings['dailyPortalFiller'].enabled = false;
				autoTrimpSettings['dailyPortalFiller'].enabledU2 = tempSettings.RFillerRun;

				autoTrimpSettings['dailyPortal'].value = tempSettings.AutoPortalDaily;
				autoTrimpSettings['dailyPortal'].valueU2 = tempSettings.RAutoPortalDaily;

				autoTrimpSettings['dailyHeliumHourChallenge'].selected = tempSettings.dHeliumHourChallenge;
				autoTrimpSettings['dailyHeliumHourChallenge'].selectedU2 = tempSettings.RdHeliumHourChallenge;

				autoTrimpSettings['dailyC2Challenge'].selected = tempSettings.dC2Challenge;
				autoTrimpSettings['dailyC2Challenge'].selectedU2 = tempSettings.RdC2Challenge;

				autoTrimpSettings['dailyPortalZone'].value = tempSettings.dCustomAutoPortal;
				autoTrimpSettings['dailyPortalZone'].valueU2 = tempSettings.RdCustomAutoPortal;

				autoTrimpSettings['dailyDontPortalBefore'].value = tempSettings.dHeHrDontPortalBefore;
				autoTrimpSettings['dailyDontPortalBefore'].valueU2 = tempSettings.RdHeHrDontPortalBefore;

				autoTrimpSettings['dailyHeliumHrBuffer'].value = tempSettings.dHeliumHrBuffer;
				autoTrimpSettings['dailyHeliumHrBuffer'].valueU2 = tempSettings.RdHeliumHrBuffer;

				autoTrimpSettings['dailyPortalSettingsArray'].valueU2 = tempSettings.rDailyPortalSettingsArray;

				autoTrimpSettings['bloodthirstDestack'].enabledU2 = tempSettings.rBloodthirstDestack;
				autoTrimpSettings['bloodthirstVoidMax'].enabledU2 = tempSettings.rBloodthirstVoidMax;
				autoTrimpSettings['empowerAutoEquality'].enabledU2 = tempSettings.rAutoEqualityEmpower;

				autoTrimpSettings['dailyDontCap'].enabled = tempSettings.dontCapDailies;
				autoTrimpSettings['dailyDontCap'].enabledU2 = tempSettings.dontCapDailies;

			}

			//Jobs
			const displayJobChanges = true; //Done!
			if (displayJobChanges) {
				autoTrimpSettings['jobType'].value = tempSettings.BuyJobsNew;
				autoTrimpSettings['jobType'].valueU2 = tempSettings.RBuyJobsNew;

				autoTrimpSettings['jobSettingsArray'].value = tempSettings.hJobSettingsArray.value;
				autoTrimpSettings['jobSettingsArray'].valueU2 = tempSettings.rJobSettingsArray.value;
			}

			//Buildings
			const displayBuildingsChanges = true; //Done
			if (displayBuildingsChanges) {
				autoTrimpSettings['buildingsType'].enabled = tempSettings.BuyBuildingsNew;
				autoTrimpSettings['buildingsType'].enabledU2 = tempSettings.RBuyBuildingsNew;

				autoTrimpSettings['buildingSettingsArray'].value = tempSettings.hBuildingSettingsArray.value;
				autoTrimpSettings['buildingSettingsArray'].valueU2 = tempSettings.rBuildingSettingsArray.value;
			}

			//C2
			const displayC2Changes = true; //Done
			if (displayC2Changes) {
				autoTrimpSettings['c2Finish'].value = tempSettings.FinishC2;
				autoTrimpSettings['c2Finish'].valueU2 = tempSettings.FinishC3;

				autoTrimpSettings['c2RunnerStart'].enabled = tempSettings.c2runnerstart;
				autoTrimpSettings['c2RunnerStart'].enabledU2 = tempSettings.c3runnerstart;

				autoTrimpSettings['c2RunnerPortal'].value = tempSettings.c2runnerpercent;
				autoTrimpSettings['c2RunnerPortal'].valueU2 = tempSettings.c3runnerpercent;

				autoTrimpSettings['c2RunnerPercent'].value = tempSettings.c2runnerpercent;
				autoTrimpSettings['c2RunnerPercent'].valueU2 = tempSettings.c3runnerpercent;

				autoTrimpSettings['unbalance'].enabledU2 = tempSettings.rUnbalance;
				autoTrimpSettings['unbalanceZone'].valueU2 = tempSettings.rUnbalanceZone;
				autoTrimpSettings['unbalanceStacks'].valueU2 = tempSettings.rUnbalanceStacks;
				autoTrimpSettings['unbalanceImprobDestack'].enabledU2 = tempSettings.rUnbalanceImprobDestack;

				autoTrimpSettings['trappapalooza'].enabledU2 = tempSettings.rTrappa;
				autoTrimpSettings['trappapaloozaCoords'].valueU2 = tempSettings.rTrappaCoords;

				autoTrimpSettings['quest'].enabledU2 = tempSettings.rQuest;
				autoTrimpSettings['questSmithyZone'].valueU2 = tempSettings.rQuestSmithyZone;

				autoTrimpSettings['mayhem'].enabledU2 = tempSettings.rMayhem;
				autoTrimpSettings['mayhemDestack'].valueU2 = tempSettings.rMayhemDestack;
				autoTrimpSettings['mayhemZone'].valueU2 = tempSettings.rMayhemZone;
				autoTrimpSettings['mayhemMapIncrease'].valueU2 = tempSettings.rMayhemMapIncrease;

				autoTrimpSettings['storm'].enabledU2 = tempSettings.Rstormon;
				autoTrimpSettings['stormZone'].valueU2 = tempSettings.rStormZone;
				autoTrimpSettings['stormStacks'].valueU2 = tempSettings.rStormStacks;

				autoTrimpSettings['pandemonium'].enabledU2 = tempSettings.RPandemoniumOn;
				autoTrimpSettings['pandemoniumZone'].valueU2 = tempSettings.RPandemoniumZone;
				autoTrimpSettings['pandemoniumAE'].valueU2 = tempSettings.RPandemoniumAutoEquip;
				autoTrimpSettings['pandemoniumAEZone'].valueU2 = tempSettings.RPandemoniumAEZone;
				autoTrimpSettings['pandemoniumFarmLevel'].valueU2 = tempSettings.PandemoniumFarmLevel;
				autoTrimpSettings['pandemoniumStaff'].valueU2 = tempSettings.RhsPandStaff;
				autoTrimpSettings['pandemoniumMP'].valueU2 = tempSettings.RPandemoniumMP;

				autoTrimpSettings['glass'].enabledU2 = tempSettings.rGlass;
				autoTrimpSettings['glassStacks'].valueU2 = tempSettings.rGlassStacks;

				autoTrimpSettings['smithless'].enabledU2 = tempSettings.rSmithless;

				autoTrimpSettings['wither'].enabledU2 = tempSettings.rWither;
			}

			//Heirlooms
			const displayHeirloomChanges = true; //Done
			if (displayHeirloomChanges) {
				autoTrimpSettings['heirloom'].enabled = tempSettings.Hhs;
				autoTrimpSettings['heirloom'].enabledU2 = tempSettings.Rhs;

				autoTrimpSettings['heirloomMapSwap'].enabled = tempSettings.HhsMapSwap;
				autoTrimpSettings['heirloomMapSwap'].enabledU2 = tempSettings.RhsMapSwap;

				autoTrimpSettings['heirloomVoidSwap'].enabledU2 = tempSettings.RhsVoidSwap;

				autoTrimpSettings['heirloomShield'].enabled = tempSettings.HhsShield;
				autoTrimpSettings['heirloomShield'].enabledU2 = tempSettings.RhsShield;

				autoTrimpSettings['heirloomInitial'].value = tempSettings.HhsInitial;
				autoTrimpSettings['heirloomInitial'].valueU2 = tempSettings.RhsInitial;

				autoTrimpSettings['heirloomAfterpush'].value = tempSettings.HhsAfterpush;
				autoTrimpSettings['heirloomAfterpush'].valueU2 = tempSettings.RhsAfterpush;

				autoTrimpSettings['heirloomC3'].value = tempSettings.HhsC3;
				autoTrimpSettings['heirloomC3'].valueU2 = tempSettings.RhsC3;

				autoTrimpSettings['heirloomSwapZone'].value = tempSettings.HhsSwapZone;
				autoTrimpSettings['heirloomSwapZone'].valueU2 = tempSettings.RhsSwapZone;

				autoTrimpSettings['heirloomSwapZoneDaily'].value = tempSettings.HhsDailySwapZone;
				autoTrimpSettings['heirloomSwapZoneDaily'].valueU2 = tempSettings.RhsDailySwapZone;

				autoTrimpSettings['heirloomSwapZoneC3'].value = tempSettings.HhsC3SwapZone;
				autoTrimpSettings['heirloomSwapZoneC3'].valueU2 = tempSettings.RhsC3SwapZone;

				autoTrimpSettings['heirloomStaff'].enabled = tempSettings.HhsStaff;
				autoTrimpSettings['heirloomStaff'].enabledU2 = tempSettings.RhsStaff;

				autoTrimpSettings['heirloomStaffWorld'].value = tempSettings.HhsWorldStaff;
				autoTrimpSettings['heirloomStaffWorld'].valueU2 = tempSettings.RhsWorldStaff;

				autoTrimpSettings['heirloomStaffMap'].value = tempSettings.HhsMapStaff;
				autoTrimpSettings['heirloomStaffMap'].valueU2 = tempSettings.RhsMapStaff;

				autoTrimpSettings['heirloomStaffFood'].value = tempSettings.HhsFoodStaff;
				autoTrimpSettings['heirloomStaffFood'].valueU2 = tempSettings.RhsFoodStaff;

				autoTrimpSettings['heirloomStaffWood'].value = tempSettings.HhsWoodStaff;
				autoTrimpSettings['heirloomStaffWood'].valueU2 = tempSettings.RhsWoodStaff;

				autoTrimpSettings['heirloomStaffMetal'].value = tempSettings.HhsMetalStaff;
				autoTrimpSettings['heirloomStaffMetal'].valueU2 = tempSettings.RhsMetalStaff;

				autoTrimpSettings['heirloomResourceStaff'].valueU2 = tempSettings.RhsResourceStaff;

				autoTrimpSettings['heirloomAuto'].enabled = tempSettings.autoheirlooms;
				autoTrimpSettings['heirloomAuto'].enabledU2 = tempSettings.autoheirlooms;

				autoTrimpSettings['heirloomAutoTypeToKeep'].value = tempSettings.typetokeep;
				autoTrimpSettings['heirloomAutoTypeToKeep'].valueU2 = tempSettings.typetokeep;

				autoTrimpSettings['heirloomAutoRareToKeep'].value = tempSettings.raretokeep;
				autoTrimpSettings['heirloomAutoRareToKeep'].valueU2 = tempSettings.raretokeep;

				autoTrimpSettings['heirloomAutoOnlyPefect'].enabled = tempSettings.autoheirloomsperfect;
				autoTrimpSettings['heirloomAutoOnlyPefect'].enabledU2 = tempSettings.autoheirloomsperfect;


				autoTrimpSettings['heirloomAutoShield'].enabled = tempSettings.keepshields;
				autoTrimpSettings['heirloomAutoShield'].enabledU2 = tempSettings.keepshields;

				autoTrimpSettings['heirloomAutoShieldMod1'].value = tempSettings.slot1modsh;
				autoTrimpSettings['heirloomAutoShieldMod1'].valueU2 = tempSettings.slot1modsh;

				autoTrimpSettings['heirloomAutoShieldMod2'].value = tempSettings.slot2modsh;
				autoTrimpSettings['heirloomAutoShieldMod2'].valueU2 = tempSettings.slot2modsh;

				autoTrimpSettings['heirloomAutoShieldMod3'].value = tempSettings.slot3modsh;
				autoTrimpSettings['heirloomAutoShieldMod3'].valueU2 = tempSettings.slot3modsh;

				autoTrimpSettings['heirloomAutoShieldMod4'].value = tempSettings.slot4modsh;
				autoTrimpSettings['heirloomAutoShieldMod4'].valueU2 = tempSettings.slot4modsh;

				autoTrimpSettings['heirloomAutoShieldMod5'].value = tempSettings.slot5modsh;
				autoTrimpSettings['heirloomAutoShieldMod5'].valueU2 = tempSettings.slot5modsh;

				autoTrimpSettings['heirloomAutoShieldMod6'].value = tempSettings.slot6modsh;
				autoTrimpSettings['heirloomAutoShieldMod6'].valueU2 = tempSettings.slot6modsh;

				autoTrimpSettings['heirloomAutoShieldMod7'].value = tempSettings.slot7modsh;
				autoTrimpSettings['heirloomAutoShieldMod7'].valueU2 = tempSettings.slot7modsh;


				autoTrimpSettings['heirloomAutoStaff'].enabled = tempSettings.keepstaffs;
				autoTrimpSettings['heirloomAutoStaff'].enabledU2 = tempSettings.keepstaffs;

				autoTrimpSettings['heirloomAutoStaffMod1'].value = tempSettings.slot1modst;
				autoTrimpSettings['heirloomAutoStaffMod1'].valueU2 = tempSettings.slot1modst;

				autoTrimpSettings['heirloomAutoStaffMod2'].value = tempSettings.slot2modst;
				autoTrimpSettings['heirloomAutoStaffMod2'].valueU2 = tempSettings.slot2modst;

				autoTrimpSettings['heirloomAutoStaffMod3'].value = tempSettings.slot3modst;
				autoTrimpSettings['heirloomAutoStaffMod3'].valueU2 = tempSettings.slot3modst;

				autoTrimpSettings['heirloomAutoStaffMod4'].value = tempSettings.slot4modst;
				autoTrimpSettings['heirloomAutoStaffMod4'].valueU2 = tempSettings.slot4modst;

				autoTrimpSettings['heirloomAutoStaffMod5'].value = tempSettings.slot5modst;
				autoTrimpSettings['heirloomAutoStaffMod5'].valueU2 = tempSettings.slot5modst;

				autoTrimpSettings['heirloomAutoStaffMod6'].value = tempSettings.slot6modst;
				autoTrimpSettings['heirloomAutoStaffMod6'].valueU2 = tempSettings.slot6modst;

				autoTrimpSettings['heirloomAutoStaffMod7'].value = tempSettings.slot7modst;
				autoTrimpSettings['heirloomAutoStaffMod7'].valueU2 = tempSettings.slot7modst;




				autoTrimpSettings['heirloomAutoCore'].enabled = tempSettings.keepcores;
				autoTrimpSettings['heirloomAutoCore'].enabledU2 = false;

				autoTrimpSettings['heirloomAutoCoreMod1'].value = tempSettings.slot1modcr;
				autoTrimpSettings['heirloomAutoCoreMod1'].valueU2 = tempSettings.slot1modcr;

				autoTrimpSettings['heirloomAutoCoreMod2'].value = tempSettings.slot2modcr;
				autoTrimpSettings['heirloomAutoCoreMod2'].valueU2 = tempSettings.slot2modcr;

				autoTrimpSettings['heirloomAutoCoreMod3'].value = tempSettings.slot3modcr;
				autoTrimpSettings['heirloomAutoCoreMod3'].valueU2 = tempSettings.slot3modcr;

				autoTrimpSettings['heirloomAutoCoreMod4'].value = tempSettings.slot4modcr;
				autoTrimpSettings['heirloomAutoCoreMod4'].valueU2 = tempSettings.slot4modcr;
			}

			//Challenges
			const displayChallengeChanges = true; //Done
			if (displayChallengeChanges) {

				autoTrimpSettings['archaeology'].enabledU2 = tempSettings.Rarchon;
				autoTrimpSettings['archaeologyString1'].valueU2 = tempSettings.Rarchstring1;
				autoTrimpSettings['archaeologyString2'].valueU2 = tempSettings.Rarchstring2;
				autoTrimpSettings['archaeologyString3'].valueU2 = tempSettings.Rarchstring3;

				autoTrimpSettings['quagmireSettings'].valueU2 = tempSettings.rQuagSettings.value;
				autoTrimpSettings['quagmireDefaultSettings'].valueU2 = tempSettings.rQuagDefaultSettings.value;

				autoTrimpSettings['insanitySettings'].valueU2 = tempSettings.rInsanitySettings.value;
				autoTrimpSettings['insanityDefaultSettings'].valueU2 = tempSettings.rInsanityDefaultSettings.value;

				autoTrimpSettings['alchemySettings'].valueU2 = tempSettings.rAlchSettings.value;
				autoTrimpSettings['alchemyDefaultSettings'].valueU2 = tempSettings.rAlchDefaultSettings.value;

				autoTrimpSettings['hypothermiaSettings'].valueU2 = tempSettings.rHypoSettings.value;
				autoTrimpSettings['hypothermiaDefaultSettings'].valueU2 = tempSettings.rHypoDefaultSettings.value;
			}

			//Golden
			const displayGoldenChanges = true; //Done!
			if (displayGoldenChanges) {
				autoTrimpSettings['autoGoldenSettings'].value = tempSettings.hAutoGoldenSettings.value;
				autoTrimpSettings['autoGoldenSettings'].valueU2 = tempSettings.rAutoGoldenSettings.value;

				autoTrimpSettings['autoGoldenDailySettings'].value = tempSettings.hAutoGoldenDailySettings.value;
				autoTrimpSettings['autoGoldenDailySettings'].valueU2 = tempSettings.rAutoGoldenDailySettings.value;

				autoTrimpSettings['autoGoldenC3Settings'].value = tempSettings.hAutoGoldenC3Settings.value;
				autoTrimpSettings['autoGoldenC3Settings'].valueU2 = tempSettings.rAutoGoldenC3Settings.value;
			}

			//Display
			const displayDisplayChanges = true; //TO DO!
			if (displayDisplayChanges) {
				autoTrimpSettings['displayEnhancedGrid'].enabled = tempSettings.EnhanceGrids;
				autoTrimpSettings['displayEnhancedGrid'].enabledU2 = tempSettings.EnhanceGrids;
				autoTrimpSettings['displayHeHr'].enabled = tempSettings.showhehr;
				autoTrimpSettings['displayHeHr'].enabledU2 = tempSettings.Rshowrnhr;
				autoTrimpSettings['displayAutoMapStatus'].enabled = tempSettings.showautomapstatus;
				autoTrimpSettings['displayAutoMapStatus'].enabledU2 = tempSettings.Rshowautomapstatus;
				autoTrimpSettings['spamMessages'].value = {
					general: tempSettings.SpamGeneral.enabled,
					fragment: tempSettings.SpamFragments.enabled,
					upgrades: tempSettings.SpamUpgrades.enabled,
					equipment: tempSettings.SpamEquipment.enabled,
					maps: tempSettings.SpamMaps.enabled,
					map_Details: tempSettings.rMapRepeatCount,
					other: tempSettings.SpamOther.enabled,
					buildings: tempSettings.SpamBuilding.enabled,
					jobs: tempSettings.SpamJobs.enabled,
					zone: tempSettings.SpamGraphs.enabled,
				};
				autoTrimpSettings['spamMessages'].valueU2 = {
					general: tempSettings.SpamGeneral.enabledU2,
					fragment: tempSettings.SpamFragments.enabledU2,
					upgrades: tempSettings.SpamUpgrades.enabledU2,
					equipment: tempSettings.SpamEquipment.enabledU2,
					maps: tempSettings.SpamMaps.enabledU2,
					map_Details: tempSettings.rMapRepeatCount,
					other: tempSettings.SpamOther.enabledU2,
					buildings: tempSettings.SpamBuilding.enabledU2,
					jobs: tempSettings.SpamJobs.enabledU2,
					zone: tempSettings.SpamGraphs.enabledU2,
				};
			}

			//Spam
			const displaySpamChanges = true; //TO DO!
			if (displaySpamChanges) {
			}
	...
	for (setting in autoTrimpSettings) {
		...
		if (initialLoad) {
			...
			if (item.type === 'boolean') {
				...
				document.getElementById(item.id).setAttribute('class', 'toggleConfigBtnLocal noselect settingsBtn settingBtn' + itemEnabled);
			}
				...
				if (elem != null) {
					if (item.type == 'multitoggle') {
						...
						document.getElementById(item.id).setAttribute('class', 'toggleConfigBtnLocal noselect settingsBtn settingBtn' + itemValue);
					}
